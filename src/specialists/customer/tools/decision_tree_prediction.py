...
@register_tool('customer_decision_tree_prediction')
class DecisionTreeAssetPredictionTool(BaseTool):
    """决策树预测未来3个月资产100万+"""
    name = 'customer_decision_tree_prediction'
    description = '决策树预测未来3个月资产100万+，输出决策规则、主要分裂特征及可视化'
    parameters = [{
        'name': 'stat_month',
        'type': 'string',
        'description': '分析截止月份（YYYY-MM），默认最新',
        'required': False
    }, {
        'name': 'min_months',
        'type': 'integer',
    ...
    def call(self, params: str, **kwargs) -> str:
        ...
        tree_img_path = os.path.join(img_dir, f'customer_decision_tree_{int(time.time() * 1000)}.png')
        plt.figure(figsize=(24, 12))
        plot_tree(dt, feature_names=feature_names, class_names=['未提升', '提升'], filled=True, rounded=True, fontsize=10)
        plt.title('决策树可视化（深度=4）')
        plt.tight_layout()
        plt.savefig(tree_img_path)
        plt.close()
        tree_img_md = f'![决策树可视化]({os.path.join("image_show", os.path.basename(tree_img_path))})'
        ...
        top_feats_md = top_feats.to_markdown()

        result = (
            f"### 决策树AUC: {auc:.4f}  准确率: {acc:.4f}\n\n"
            f"### 主要分裂特征（Top10）\n{top_feats_md}\n\n"
            f"### 决策树文本规则\n"
            f"```\n{tree_text}\n```\n"
            f"{tree_img_md}"
